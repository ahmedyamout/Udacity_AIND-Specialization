...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if actionNode.prenodes.issubset(self.s_levels[level]):
                ...
                for node_s in self.s_levels[level]:
                    node_s.children.add(actionNode)
                    actionNode.parents.add(node_s)
        self.a_levels.append(actionNodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1Actions in node_a1.action.effect_add:
            if a1Actions in node_a2.action.effect_rem:
                return True
        for a2Actions in node_a2.action.effect_add:
            if a2Actions in node_a1.action.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1Action in node_a1.action.effect_add:
            if a1Action in node_a2.action.precond_neg:
                return True
        for a2Action in node_a2.action.effect_add:
            if a2Action in node_a1.action.precond_neg:
                return True
        for a1Action in node_a1.action.effect_rem:
            if a1Action in node_a2.action.precond_pos:
                return True
        for a2Action in node_a2.action.effect_rem:
            if a2Action in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1Parent in node_a1.parents:
            for a2Parent in node_a2.parents:
                if a1Parent.is_mutex(a2Parent):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1Parent in node_s1.parents:
            for s2Parent in node_s2.parents:
                if not s1Parent.is_mutex(s2Parent):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for level in range(len(self.s_levels)):
            ...
            for literal in self.s_levels[level]:
                if expr(literal.symbol) in goals:
                    level_sum += level
                    goals.remove(expr(literal.symbol))
                    break
        return level_sum